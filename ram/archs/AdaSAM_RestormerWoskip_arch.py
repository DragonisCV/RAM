...
class AdaSAM_RestormerWoskip(nn.Module):
    def __init__(self,
        ...
        super(AdaSAM_RestormerWoskip, self).__init__()
        ...
        self.refinement = nn.Sequential(*[TransformerBlock(dim=int(dim * 2**1), num_heads=heads[0], ffn_expansion_factor=ffn_expansion_factor, bias=bias,
                                        LayerNorm_type=LayerNorm_type, finetune_type=finetune_type if i == num_refinement_blocks - 1 else None) for i in range(num_refinement_blocks)])
